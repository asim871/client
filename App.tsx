...
// Standard hosting utility: ensure page starts at top on route change
const ScrollToTop = () => {
      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-8 items-center">
          ...
          <div className="flex items-center gap-5 ml-4 border-l border-white/10 pl-6">
            ...
            <Youtube className="w-4 h-4 cursor-pointer text-zinc-500 hover:text-white transition-colors" />
        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col gap-6">
              {navLinks.map((link) => (
                <Link key={link.name} to={link.path} className="text-5xl font-heading font-black text-white hover:text-blue-500 transition-colors">
                  {link.name}
                </Link>
              ))}
            </div>
            <div className="mt-12 pt-12 border-t border-white/10 flex gap-6">
               ...
               <Youtube className="text-zinc-400" />
               <Facebook className="text-zinc-400" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-16 md:gap-8">
        <div className="md:col-span-5">
          ...
          <div className="flex gap-4">
            {['Instagram', 'Facebook', 'Behance'].map(social => (
              <div key={social} className="px-5 py-2 rounded-full border border-white/10 flex items-center justify-center text-xs font-bold hover:bg-white hover:text-black transition-all cursor-pointer">
                {social}
              </div>
            ))}
          </div>
        ...
        <div className="md:col-span-2">
          ...
          <ul className="space-y-4 text-zinc-500 text-sm font-medium">
            ...
            <li><Link to="/blog" className="hover:text-blue-400 transition-colors">Insights</Link></li>
          </ul>
        </div>

        <div className="md:col-span-2">
          <h4 className="text-white text-xs font-bold uppercase tracking-[0.2em] mb-8">Legal</h4>
          <ul className="space-y-4 text-zinc-500 text-sm font-medium">
        ...
        <div className="md:col-span-3">
          <h4 className="text-white text-xs font-bold uppercase tracking-[0.2em] mb-8">Headquarters</h4>
          <ul className="space-y-4 text-zinc-500 text-sm font-medium">
            <li className="flex items-center gap-3"><Mail size={14} className="text-zinc-700" /> hello@pigeon.studio</li>
            <li className="flex items-center gap-3"><Phone size={14} className="text-zinc-700" /> +1 (555) 000-1234</li>
            <li className="flex items-start gap-3"><MapPin size={14} className="text-zinc-700 mt-1" /> 123 Motion Way, <br /> Brooklyn, NY 11201</li>
          </ul>
        </div>
      </div>
      <div className="max-w-7xl mx-auto mt-32 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-[10px] text-zinc-700 font-bold uppercase tracking-widest gap-4">
        <p>© 2024 Pigeon Studio Collective. All rights reserved.</p>
        <p className="flex items-center gap-2">Built with passion & AI <ExternalLink size={10} /></p>
      </div>
    </footer>
  );
};
